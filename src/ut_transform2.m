...
function [mu,S,C,X,Y,w] = ut_transform2(M,P,g,g_param,tr_param,constraints,key)
...
elseif mat
    ...
    if sum(key)
        X = ut_sigmas_constrained(M,P,c,constraints,key);
    else
        X = ut_sigmas(M,P,c);
    end
    w = {WM,W,c};
...
%
% Propagate through the model function
%
if isnumeric(g)
    Y = g*X;
elseif ischar(g) || isa(g,'function_handle')
    Y = [];
    for i=1:size(X,2)                           % loop through all the sigma vectors
...
if mat
    ...
    N_params = g_param.N_params;
    g_param.mean_est = 1;
    mu_x = g(M,g_param);
    g_param.mean_est = 0;
    mu_lambda = X(end-N_params+1:end,:)*WM;
    mu = [mu_x(1:end-N_params) ; mu_lambda];
    ...
    if sum(key)
        for n = 1:length(key)
            mu_bound = mu(key(n),:);
            mu_bound(mu_bound < constraints(n,1)) = constraints(n,1);
            mu_bound(mu_bound > constraints(n,2)) = constraints(n,2);
            mu(key(n),:) = mu_bound;
        end
    end
    ...
    for i=1:size(X,2)
        ...
        C = C + WC(i) * (X(1:size(M,1),i) - M) * (Y(:,i) - mu)';      %***** unused with linear update
    end
end
