...
function [M,P,K] = ukf_update1(M,P,Y,h,R,h_param,alpha,beta,kappa,mat)
...
%** linear kalman update equations**    %?????? H is linear, can do linear
                                        %update. not sure about
                                        %alpha/beta/kappa here
I = eye(size(P));
K = P*h'/(h*P*h' + R); %K = P * (h'/R);
M = M + K*(Y - h*M);
P = (I-K*h)*P*(I-K*h)' + K*R*K'; %(I - K*h)*P;  %P = inv(inv(P) + h'/R*h);%update covariance (multiple possible eqs here)
P = (P+P')/2;  %symmetrise P
